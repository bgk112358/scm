...
int SF_CertificateManagement() {
    ...
    // 获取证书链
    int rv = CY_GetTrustedCertificateChain(hAppHandle, NULL, &uiBufferLen);
    if (rv != 0) {
        read_file("../../examples/Assets/cert_chain.crt", &pucBuffer, &uiBufferLen);
        // 导入证书链
        rv = CY_ImportTrustedCertificateChain(hAppHandle, pucBuffer, uiBufferLen);
    ...
    read_file("../../examples/Assets/client.crt", &pucBuffer, &uiBufferLen);
    ...
    if (pucBuffer) free(pucBuffer);
    if (rv != CY_R_SUCCESS) {
        printf("CY_ImportCertificate error, error code: %02x\n", rv);
        return rv;
    } else {
        printf("CY_ImportCertificate success\n");
    }

    rv = CY_GetCertificateStatus(hAppHandle,
        pucCyberContainerName,
        uiCyberContainerNameLen,
        pucPinInfo,
        uiPinInfoLen);
    ...
    rv = CY_ExportCertificate(
            ...
            &uiBufferLen);
    if (rv != CY_R_SUCCESS) {
        printf("CY_ExportCertificate error, error code: %02x\n", rv);
        return rv;
    } else {
        printf("CY_ExportCertificate success\n");
        hex_print(pucBuffer, uiBufferLen);
    }
